...
class TitleState extends MusicBeatState
{
	...
	public static var trickrandom:Int = 99;
	...
	override public function create():Void
	{
		...
		clay = new FlxSprite(250, 400).loadGraphic(Paths.image('clay','clown'));
		clay.antialiasing = true;
		clay.setGraphicSize(Std.int(clay.width * 0.22));
		clay.updateHitbox();

		var random = FlxG.random.int(0,1);
		if (random == trickrandom) {
			random++;
			if (random > 1) 
				random = 0;
		}
		switch (random) 
		{
			...
			case 1:
				gfDance = new FlxSprite(FlxG.width * 0.23, FlxG.height * 0.07);
				gfDance.frames = Paths.getSparrowAtlas('DJ_Tiky','clown');
				gfDance.animation.addByPrefix('dance', 'mixtape',24, true);
				gfDance.antialiasing = true;
				gfDance.setGraphicSize(Std.int(gfDance.width * 0.6));
		}
	...
	{
		Timer += 11;
        clay.angle = Math.sin(Timer / 270) * 6;
